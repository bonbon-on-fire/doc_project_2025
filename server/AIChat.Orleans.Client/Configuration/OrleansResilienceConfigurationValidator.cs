...
/// </summary>
public partial class OrleansResilienceConfigurationValidator
    : IValidateOptions<OrleansResilienceConfiguration>
{
    ...
    )
    {
        ...
        {
            failures.Add(
                $"CircuitBreaker.FailureThreshold must be between 1 and 20, but was {settings.FailureThreshold}"
            );
        }
        ...
        {
            failures.Add(
                $"CircuitBreaker.SamplingDurationSeconds must be between 10 and 300, but was {settings.SamplingDurationSeconds}"
            );
        }
        ...
        {
            failures.Add(
                $"CircuitBreaker.MinimumThroughput must be between 1 and 100, but was {settings.MinimumThroughput}"
            );
        }
        ...
        {
            failures.Add(
                $"CircuitBreaker.BreakDurationSeconds must be between 5 and 120, but was {settings.BreakDurationSeconds}"
            );
        }
    }
    ...
    private static void ValidateRetryPolicy(RetryPolicySettings settings, List<string> failures)
    {
        ...
        {
            failures.Add(
                $"RetryPolicy.MaxRetryAttempts must be between 1 and 10, but was {settings.MaxRetryAttempts}"
            );
        }
        ...
        {
            failures.Add(
                $"RetryPolicy.BaseDelayMilliseconds must be between 10 and 5000, but was {settings.BaseDelayMilliseconds}"
            );
        }
        ...
        {
            failures.Add(
                $"RetryPolicy.MaxDelayMilliseconds must be between 100 and 60000, but was {settings.MaxDelayMilliseconds}"
            );
        }
        ...
        {
            failures.Add(
                $"RetryPolicy.MaxDelayMilliseconds ({settings.MaxDelayMilliseconds}) must be greater than BaseDelayMilliseconds ({settings.BaseDelayMilliseconds})"
            );
        }
        ...
        if (settings.RetryableExceptions.Count > 0)
        {
            foreach (var exceptionType in settings.RetryableExceptions)
            {
                ...
                {
                    failures.Add(
                        "RetryPolicy.RetryableExceptions cannot contain null or empty exception type names"
                    );
                    break;
                }
                ...
                {
                    failures.Add(
                        $"RetryPolicy.RetryableExceptions contains invalid type name: '{exceptionType}'"
                    );
                }
            }
        }
    ...
    private static void ValidateTimeout(TimeoutSettings settings, List<string> failures)
    {
        ...
        {
            failures.Add(
                $"Timeout.DefaultTimeoutSeconds must be between 1 and 300, but was {settings.DefaultTimeoutSeconds}"
            );
        }
        ...
        {
            failures.Add(
                $"Timeout.HealthCheckTimeoutSeconds must be between 1 and 30, but was {settings.HealthCheckTimeoutSeconds}"
            );
        }
        ...
        {
            failures.Add(
                $"Timeout.StateRetrievalTimeoutSeconds must be between 1 and 60, but was {settings.StateRetrievalTimeoutSeconds}"
            );
        }
        ...
        {
            failures.Add(
                $"Timeout.HealthCheckTimeoutSeconds ({settings.HealthCheckTimeoutSeconds}) should not exceed DefaultTimeoutSeconds ({settings.DefaultTimeoutSeconds})"
            );
        }
        ...
        {
            failures.Add(
                $"Timeout.StateRetrievalTimeoutSeconds ({settings.StateRetrievalTimeoutSeconds}) should not exceed DefaultTimeoutSeconds ({settings.DefaultTimeoutSeconds})"
            );
        }
    }
    ...
    private static void ValidateBulkhead(BulkheadSettings settings, List<string> failures)
    {
        ...
        {
            failures.Add(
                $"Bulkhead.MaxConcurrency must be between 1 and 100, but was {settings.MaxConcurrency}"
            );
        }
        ...
        {
            failures.Add(
                $"Bulkhead.MaxQueuedItems must be between 0 and 200, but was {settings.MaxQueuedItems}"
            );
        }
        ...
        {
            failures.Add(
                $"Bulkhead.MaxQueuedItems ({settings.MaxQueuedItems}) is excessively large compared to MaxConcurrency ({settings.MaxConcurrency}). Consider reducing queue size."
            );
        }
    }
    ...
    private static bool IsValidTypeName(string typeName)
    {
        ...
        return parts.Length >= 2
            && parts.All(part => !string.IsNullOrWhiteSpace(part) && MyRegex().IsMatch(part));
    }
