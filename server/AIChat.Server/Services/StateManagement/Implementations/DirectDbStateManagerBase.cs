...
/// <summary>
/// Base class for direct database-based state managers.
/// Provides common functionality for state managers that use direct database access
/// through existing storage interfaces (IChatStorage, IModeStorage, etc.).
/// </summary>
/// <typeparam name="T">The type of entity being managed</typeparam>
public abstract class DirectDbStateManagerBase<T> : IStateManager<T> where T : class
{
    /// <summary>
    /// Gets the logger instance for this state manager.
    /// </summary>
    protected ILogger<DirectDbStateManagerBase<T>> Logger { get; }
    ...
    /// <summary>
    /// Initializes a new instance of the DirectDbStateManagerBase class.
    /// </summary>
    /// <param name="logger">The logger instance</param>
    /// <param name="cacheManager">The cache manager</param>
    protected DirectDbStateManagerBase(
        ILogger<DirectDbStateManagerBase<T>> logger,
    ...
    public async Task<StateResult<T>> GetAsync(string id, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            Logger.LogDebug("Getting entity {EntityType} with ID {EntityId} from direct DB (CorrelationId: {CorrelationId})",
        ...
        {
            Logger.LogError(ex, "Error getting {EntityType} with ID {EntityId} from direct DB (CorrelationId: {CorrelationId})",
                typeof(T).Name, id, correlationId);
    ...
    public async Task<StateResult<IReadOnlyList<T>>> GetMultipleAsync(IEnumerable<string> ids, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            Logger.LogDebug("Getting multiple entities {EntityType} with {Count} IDs from direct DB (CorrelationId: {CorrelationId})",
                typeof(T).Name, idList.Count, correlationId);

            var result = await GetMultipleFromDatabaseAsync(idList, cancellationToken);
        ...
        {
            Logger.LogError(ex, "Error getting multiple {EntityType} entities from direct DB (CorrelationId: {CorrelationId})",
                typeof(T).Name, correlationId);
    ...
    public async Task<StateResult<PagedResult<T>>> GetPagedAsync(StateQuery query, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            Logger.LogDebug("Getting paged entities {EntityType} from direct DB (Page: {Page}, PageSize: {PageSize}, CorrelationId: {CorrelationId})",
                typeof(T).Name, query.Page, query.PageSize, correlationId);

            var result = await GetPagedFromDatabaseAsync(query, cancellationToken);
        ...
        {
            Logger.LogError(ex, "Error getting paged {EntityType} entities from direct DB (CorrelationId: {CorrelationId})",
                typeof(T).Name, correlationId);
    ...
    public async Task<StateResult<bool>> ExistsAsync(string id, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            Logger.LogDebug("Checking existence of {EntityType} with ID {EntityId} in direct DB (CorrelationId: {CorrelationId})",
                typeof(T).Name, id, correlationId);
        ...
        {
            Logger.LogError(ex, "Error checking existence of {EntityType} with ID {EntityId} in direct DB (CorrelationId: {CorrelationId})",
                typeof(T).Name, id, correlationId);
    ...
    public async Task<StateResult<long>> CountAsync(StateQuery? query = null, CancellationToken cancellationToken = default)
    {
        ...
        {
            Logger.LogError(ex, "Error counting {EntityType} entities in direct DB (CorrelationId: {CorrelationId})",
                typeof(T).Name, correlationId);
    ...
    public async Task<StateResult<T>> CreateAsync(T entity, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            Logger.LogDebug("Creating entity {EntityType} in direct DB (CorrelationId: {CorrelationId})",
                typeof(T).Name, correlationId);

            var result = await CreateInDatabaseAsync(entity, cancellationToken);
        ...
        {
            Logger.LogError(ex, "Error creating {EntityType} entity in direct DB (CorrelationId: {CorrelationId})",
                typeof(T).Name, correlationId);
    ...
    public async Task<StateResult<T>> UpdateAsync(string id, T entity, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            Logger.LogDebug("Updating entity {EntityType} with ID {EntityId} in direct DB (CorrelationId: {CorrelationId})",
                typeof(T).Name, id, correlationId);

            var result = await UpdateInDatabaseAsync(id, entity, cancellationToken);
        ...
        {
            Logger.LogError(ex, "Error updating {EntityType} with ID {EntityId} in direct DB (CorrelationId: {CorrelationId})",
                typeof(T).Name, id, correlationId);
    ...
    public async Task<StateResult<T>> PatchAsync(string id, Dictionary<string, object> updates, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            Logger.LogDebug("Patching entity {EntityType} with ID {EntityId} in direct DB (CorrelationId: {CorrelationId})",
                typeof(T).Name, id, correlationId);

            var result = await PatchInDatabaseAsync(id, updates, cancellationToken);
        ...
        {
            Logger.LogError(ex, "Error patching {EntityType} with ID {EntityId} in direct DB (CorrelationId: {CorrelationId})",
                typeof(T).Name, id, correlationId);
    ...
    public async Task<StateResult> DeleteAsync(string id, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            Logger.LogDebug("Deleting entity {EntityType} with ID {EntityId} from direct DB (CorrelationId: {CorrelationId})",
                typeof(T).Name, id, correlationId);

            var result = await DeleteInDatabaseAsync(id, cancellationToken);
        ...
        {
            Logger.LogError(ex, "Error deleting {EntityType} with ID {EntityId} from direct DB (CorrelationId: {CorrelationId})",
                typeof(T).Name, id, correlationId);
    ...
    public virtual async Task<StateManagerHealthStatus> CheckHealthAsync(CancellationToken cancellationToken = default)
    {
        try
        {
            // Basic health check by attempting a simple database operation
            ...
            return StateManagerHealthStatus.Healthy("DirectDB", new Dictionary<string, object>
            {
        ...
        {
            Logger.LogError(ex, "Health check failed for direct DB state manager");
            return StateManagerHealthStatus.Unhealthy("DirectDB", ex.Message);
        }
    }
    ...
    /// <summary>
    /// Gets an entity from the database using storage interfaces.
    /// </summary>
    protected abstract Task<StateResult<T>> GetFromDatabaseAsync(string id, CancellationToken cancellationToken);

    /// <summary>
    /// Gets multiple entities from the database.
    /// </summary>
    protected abstract Task<StateResult<IReadOnlyList<T>>> GetMultipleFromDatabaseAsync(IList<string> ids, CancellationToken cancellationToken);

    /// <summary>
    /// Gets a paged list of entities from the database.
    /// </summary>
    protected abstract Task<StateResult<PagedResult<T>>> GetPagedFromDatabaseAsync(StateQuery query, CancellationToken cancellationToken);

    /// <summary>
    /// Checks if an entity exists in the database.
    /// </summary>
    protected abstract Task<StateResult<bool>> ExistsInDatabaseAsync(string id, CancellationToken cancellationToken);

    /// <summary>
    /// Counts entities in the database.
    /// </summary>
    protected abstract Task<StateResult<long>> CountInDatabaseAsync(StateQuery? query, CancellationToken cancellationToken);

    /// <summary>
    /// Creates an entity in the database.
    /// </summary>
    protected abstract Task<StateResult<T>> CreateInDatabaseAsync(T entity, CancellationToken cancellationToken);

    /// <summary>
    /// Updates an entity in the database.
    /// </summary>
    protected abstract Task<StateResult<T>> UpdateInDatabaseAsync(string id, T entity, CancellationToken cancellationToken);

    /// <summary>
    /// Patches an entity in the database.
    /// </summary>
    protected abstract Task<StateResult<T>> PatchInDatabaseAsync(string id, Dictionary<string, object> updates, CancellationToken cancellationToken);

    /// <summary>
    /// Deletes an entity from the database.
    /// </summary>
    protected abstract Task<StateResult> DeleteInDatabaseAsync(string id, CancellationToken cancellationToken);
