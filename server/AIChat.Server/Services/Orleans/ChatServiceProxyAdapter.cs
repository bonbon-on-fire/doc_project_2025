...
/// <summary>
/// HTTP-based adapter that bridges between Orleans grains and the Server API.
/// This adapter makes HTTP calls to Server endpoints to provide real LLM processing capability to Orleans grains.
/// Avoids circular dependencies by using HTTP communication instead of direct service dependencies.
/// </summary>
public class HttpChatServiceProxyAdapter : IChatServiceProxy
{
    private readonly HttpClient _httpClient;
    private readonly ILogger<HttpChatServiceProxyAdapter> _logger;
    private readonly JsonSerializerOptions _jsonOptions;

    /// <summary>
    /// Initializes a new instance of the HttpChatServiceProxyAdapter.
    /// </summary>
    /// <param name="httpClient">HTTP client for making API calls to Server</param>
    /// <param name="logger">Logger for diagnostics</param>
    public HttpChatServiceProxyAdapter(HttpClient httpClient, ILogger<HttpChatServiceProxyAdapter> logger)
    {
    ...
    )
    {
        _logger.LogInformation(
            "Processing chat request through HTTP API. ChatId: {ChatId}, UserId: {UserId}, RequestId: {RequestId}",
            request.ChatId,
            request.UserId,
            request.RequestId
        );
        ...
        try
        {
            ...
            if (processingException != null)
            {
                _logger.LogError(processingException,
                    "Error making HTTP request for ChatId: {ChatId}, RequestId: {RequestId}",
                    request.ChatId,
                    request.RequestId
                );
            ...
            {
                _logger.LogError(
                    "HTTP request failed for ChatId: {ChatId}. StatusCode: {StatusCode}",
                    request.ChatId,
                    response?.StatusCode
                );
            ...
            if (responseException != null)
            {
                _logger.LogError(responseException,
                    "Error processing HTTP response for ChatId: {ChatId}",
                    request.ChatId
                );
            ...
            if (!string.IsNullOrEmpty(responseJson))
            {
                ...
                _logger.LogInformation(
                    "Completed HTTP-based chat request for ChatId: {ChatId}. Total chunks: {ChunkCount}",
                    request.ChatId,
                    chunkIndex + 1
                );
            }
