...
/// </summary>
public class WeatherFunction(ILogger<WeatherFunction> logger) : IFunctionProvider
{
    ...
    {
        logger.LogInformation("GetWeather called with args: {Args}", argsJson);
        ...
        {
            logger.LogError(ex, "Error in GetWeather function");
    ...
    {
        logger.LogInformation("Calculate called with args: {Args}", argsJson);
        ...
        {
            logger.LogError(ex, "Error in Calculate function");
            return JsonSerializer.Serialize(new { error = ex.Message });
        }
    }
